...
try:
    with open("project-files/insert_sql_queries.sql", "w") as f:
        ...
        for lecturer in lecturers:
            lec_id = lecturer[0]
            available_courses = [course for course in courses if course[1] not in assigned_courses] # Get unassigned courses only
            if not available_courses:
                break  # No more courses left to assign
            num_courses = min(random.randint(1, 5), len(available_courses))  # Assign a random number of courses, up to the number of available courses
        ...
        for course in courses:
            ...
            for i in range(1,4):
                ...
                forum_id += 1
                print(f" Statement#{num}: Insert statements created for discussion forum")
                num += 1
        ...
        for forum in discussion_forum:
            ...
            for i in range(1,5):
                ...
                thread_id += 1
                print(f" Statement#{num}: Insert statements created for discussion thread")
                num += 1
        ...
        for thread in discussion_thread:
            ...
            for i in range(1,4):
                ...
                if reply_to is None:
                    ...
                    print(f" Statement#{num}: Insert statements created for reply")
                    num += 1
                else:
